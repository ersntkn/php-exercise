...
while($data = mysqli_fetch_array($result)){
  ...
  if (($data["isActive"]) == 1) {$isActive = "checked";}else {$isActive = "";}
    ...
    $tableBody .="<tr class='".$oddEven." text-center'><td class='dtr-control sorting_1' tabindex='0'>".$data["title"]."</td><td>".$data["whoIsAdd"]."</td><td>".$data["description"]."</td>
      ...
      <td>
          <div class='form-group'>
            <div class='custom-control custom-switch'>
              <input type='checkbox' class='custom-control-input' ".$isActive ." id='".$data["id"]."'>
      ...
      <td><a class='btn btn-sm btn-warning fillArticleModal' data-toggle='modal' data-target='#updateArticleModal' value='".$data["id"]."'><i class='fas fa-pen'></i> Edit</a><a class='btn btn-sm btn-danger deleteArticle ml-2'  value='".$data["id"]."'><i class='fas fa-trash'></i> Delete</a></td></tr>";
    $count++;
}
